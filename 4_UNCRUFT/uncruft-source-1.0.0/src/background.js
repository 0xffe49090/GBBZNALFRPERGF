...
function applyRules() {
  applyQueue = applyQueue.catch(() => undefined).then(() => applyRulesNow());
  return applyQueue;
}

ext.runtime.onInstalled.addListener(() => {
  applyRules().catch(error => console.error("Uncruft install:", error));
});
if (ext.runtime.onStartup) {
  ext.runtime.onStartup.addListener(() => {
    applyRules().catch(error => console.error("Uncruft startup:", error));
  });
}
ext.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type !== "apply-rules") return undefined;
  applyRules()
